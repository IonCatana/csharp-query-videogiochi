...
SELECT COUNT(*) AS [tournament in 2015]
FROM tournaments
WHERE year = 2015;
...
SELECT COUNT(*) AS [software houses], country 
FROM software_houses 
GROUP BY country 
ORDER BY [software houses] DESC;
-- 
-- 2- Contare quante recensioni ha ricevuto ogni videogioco (del videogioco vogliamo solo l'ID) (500)
SELECT videogame_id, COUNT(*) AS [reviews number]
FROM reviews
...
SELECT YEAR(release_date) AS [year], COUNT(*) AS [videogames number] 
FROM videogames
GROUP BY YEAR(release_date);
-- 
-- 5- Contare quanti videogiochi sono disponbiili per ciascun device (del device vogliamo solo l'ID) (7)
SELECT device_id, COUNT (*) AS videogames
FROM device_videogame 
GROUP BY device_id;
-- 
-- 6- Ordinare i videogame in base alla media delle recensioni (del videogioco vogliamo solo l'ID) (500)
SELECT videogame_id, AVG(CAST(rating AS decimal(10,2))) AS ratingAVG
FROM reviews 
GROUP BY videogame_id
